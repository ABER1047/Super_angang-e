...
attack ++
	...
	var bl_ef = instance_create_depth(x-24*image_xscale,y,depth-1,ef_blood)
	var img_scale = irandom_range(8,15)/10
	bl_ef.image_xscale = img_scale
	bl_ef.image_yscale = abs(img_scale)
	bl_ef.t_x = 0
	//bl_ef.image_blend = $FF5B14EE
	}
	...
	image_index = 2
	attack = 0
	var effect = instance_create_depth(x,y,depth+1,jan_sang_critical)
...
if cool_time = 0
{
	if instance_exists(enemy_mob_parents)
	{
	target = enemy_mob_parents
	}
}


if attack = 0 && instance_exists(enemy_mob_parents) && abs(x - enemy_mob_parents.x) < 32 && can_attack = 1
{
var random_attack = percentage_k(20)
	...
	{
	var effect = instance_create_depth(x,y,depth+1,jan_sang)
	effect.sprite_index = sprite_index
	effect.image_speed = 0
	effect.image_index = image_index
	gravity = 0
...
var obj_m = instance_place(x,y,mob_parents)
	if instance_exists(obj_m) && cool_time = 0 
	{
		...
		cool_time = 1
		alarm[1] = 30
			if abs(t_movement_speed) < obj_m.t_movement_speed
			{
...
if place_meeting(x,y,jan_sang_enemy) && hurt = 0
{
var obj_m = instance_place(x,y,jan_sang_enemy)
...
var random_voice = percentage_k(40)
...
if place_meeting(x,y,jan_sang_enemy_critical) && hurt = 0
{
var obj_m = instance_place(x,y,jan_sang_enemy)
var sfx = audio_play_sound(choose(hit_sfx1,hit_sfx2,hit_sfx3),0,0)
audio_sound_gain(sfx,0.35*global.master_volume*global.sfx_volume,0)
...
var sfx = audio_play_sound(choose(sibalruna,wakgood_hurt,jyunne_a_i_upne),0,0)
audio_sound_gain(sfx,0.35*global.master_volume*global.sfx_volume,0)
